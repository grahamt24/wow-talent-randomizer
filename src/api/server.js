...
app.post('/download-image', async (req, res) => {
  ...
  try {
    const { data: { id, media } } = await axios.get(`${url}&access_token=${accessToken}`);
    const mediaUrl = `${media.key.href}&access_token=${accessToken}`;
    const { data: { assets } } = await axios.get(mediaUrl);
    const fileUrl = assets[0].value;
    const dirPath = path.join(__dirname, `../assets/${className}/talents/${specName}/`);
    ...
    res.status(200).send({ message: 'Image downloaded successfully', filePath });
  } catch (error) {
